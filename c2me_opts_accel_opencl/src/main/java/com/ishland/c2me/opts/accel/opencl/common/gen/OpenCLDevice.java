...
import com.ishland.c2me.opts.accel.opencl.common.scheduler.SingleThreadExecutor;
...
public class OpenCLDevice implements Closeable {
   ...
   public OpenCLDevice(CLServerGlobalContext globalContext, OpenCLDeviceMetadata metadata) {
      ...
      this.deviceDescription = String.format("OpenCL Device %s (%s)", CLUtil.getDeviceInfoStringUTF8(metadata.devicePtr, 4139), metadata.deviceUUID);
      ...
      CompletableFuture.runAsync(
            () -> {
               ...
               try {
                  ...
                  ctxProps.put(4228L).put(metadata.platformPtr);
                  ctxProps.rewind();
                  this.callback = CLContextCallback.create(
                     (errinfo, private_info, cb, user_data) -> LOGGER.error("OpenCL Error on {}: {}", this.deviceDescription, MemoryUtil.memUTF8(errinfo))
                  );

                  try {
                     this.context = CL12.clCreateContext(ctxProps, metadata.devicePtr, this.callback, 0L, errorCodeRet);
                     ...
                     try {
                        ...
                        CLUtil.checkCLError(CL12.clGetDeviceInfo(metadata.devicePtr, 4138, property, null));
                        boolean supportsOutOfOrder = (property.get() & 1L) != 0L;
                        ...
                        queueProp.put(4243L).put(supportsOutOfOrder ? 1L : 0L);
                        ...
                        try {
                           if (useQueueProperties) {
                              this.commandQueue = CL20.clCreateCommandQueueWithProperties(this.context, metadata.devicePtr, queueProp.rewind(), errorCodeRet);
                              CLUtil.checkCLError(errorCodeRet);
                           } else {
                              this.commandQueue = CL12.clCreateCommandQueue(this.context, metadata.devicePtr, supportsOutOfOrder ? 1L : 0L, errorCodeRet);
                              CLUtil.checkCLError(errorCodeRet);
                           }
         ...
         .join();
      FlowschedAssertions.assertTrue(this.context != 0L, "Failed to create OpenCL context for " + this.deviceDescription);
      FlowschedAssertions.assertTrue(this.callback != null, "Failed to create OpenCL context callback for " + this.deviceDescription);
   }
         ...
         () -> {
            ...
            try {
               ...
               if (this.metadata.deviceCaps.cl_intel_device_attribute_query) {
                  int ipVersion = CLUtil.getDeviceInfoInt(this.metadata.devicePtr, 16976);
                  if (ipVersion == 589824) {
                     this.globalContext
                        .shaderCacheManager
                        .tryCacheDirs(this.context, this.metadata, map, String.format("%s/%s/%s/Gen9", sha256, "intel", osKey));
                  } else {
                     this.globalContext
                        .shaderCacheManager
                        .tryCacheDirs(this.context, this.metadata, map, String.format("%s/%s/%s/%d", sha256, "intel", osKey, ipVersion));
                  }
               ...
               for (int valuesLength = values.length; i < valuesLength; i++) {
                  OpenCLCGen.ProgramType value = values[i];
                  if (!map.containsKey(value)) {
                     LOGGER.info("Compiling program {} for {} for device {} ({}/{})", new Object[]{value, desc, this, i + 1, valuesLength});
                     GlobalProgressStash.PROGRESS_TEXT = String.format(
                        "Compiling %s for %s on %s (%d/%d)", value, desc, this.deviceDescription, i + 1, valuesLength
                     );
               ...
               try {
                  ...
                  LOGGER.warn("Failed to unload OpenCL platform compiler for {}", this.deviceDescription, var11);
               }
   ...
   private void workaroundGen9Caches(String sha256, EnumMap<OpenCLCGen.ProgramType, Long> map) {
      ...
      String[] paths = Arrays.stream(gen9IPs).mapToObj(ipVersion1 -> String.format("%s/%s/%d", sha256, "intel", ipVersion1)).toArray(String[]::new);
      this.globalContext.shaderCacheManager.tryCacheDirs(this.context, this.metadata, map, paths);
   }

   private long compileProgram0(
      String desc, String source, String absoluteDumpPath, Object2ReferenceOpenHashMap<String, String> defines, OpenCLCGen.ProgramType type
   ) {
      ...
      try {
         ...
         try {
            ...
            try {
               ...
               if (!this.workarounds.contains(Workarounds.Reference.NVIDIA_FAST_COMPILE_UNAVAILABLE)
                  && Config.enableNvidiaFastCompilation
                  && this.metadata.deviceCaps.cl_nv_compiler_options) {
                  option.append("-cl-nv-opt-level=1 -cl-nv-verbose -nv-use-200772613 ");
               }

               option.append("-D DF_COMPILE_").append(type.name()).append("=1 ");
               if (defines != null) {
                  ObjectIterator t1 = defines.object2ReferenceEntrySet().iterator();

                  while (t1.hasNext()) {
                     Entry<String, String> entry = (Entry<String, String>)t1.next();
                     option.append(" -D ").append((String)entry.getKey()).append("=").append((String)entry.getValue()).append(" ");
                  }
               }

               LOGGER.info("Invoking compiler for device {} with options: {}", this, option.toString());
               int buildError = CL12.clBuildProgram(program, this.metadata.devicePtr, option.toString(), null, 0L);
               IntBuffer buildStatus = stack.callocInt(1);
               CLUtil.checkCLError(CL12.clGetProgramBuildInfo(program, this.metadata.devicePtr, 4481, buildStatus, null));
               String buildLog = null;
               PointerBuffer sizeRet = stack.callocPointer(1);
               CLUtil.checkCLError(CL12.clGetProgramBuildInfo(program, this.metadata.devicePtr, 4483, (ByteBuffer)null, sizeRet));
               if (sizeRet.get(0) != 0L) {
                  ByteBuffer log = MemoryUtil.memAlloc((int)sizeRet.get(0));

                  try {
                     CLUtil.checkCLError(CL12.clGetProgramBuildInfo(program, this.metadata.devicePtr, 4483, log, null));
               ...
               if (buildStatus.get(0) != 0) {
                  LOGGER.error("Failed to build OpenCL program for {} on {}: \n{}", new Object[]{desc, this.deviceDescription, buildLog});
   ...
   private void combinePendingAction(CompletionStage<?> stage) {
      this.pendingActions.updateAndGet(future -> future.thenCombine(stage, (var0, var1) -> (Void)null).exceptionally(var0 -> null));
   }

   private void assertOpen() {
      if (!this.open.get()) {
         throw new IllegalStateException("OpenCL device " + this.deviceDescription + " is closed");
      }
   }
   ...
   public OpenCLDevice.BorrowedCommandQueue borrowCommandQueue() {
      synchronized (this.contextMutex) {
         if (this.permits.get() > 0) {
            ...
            return new OpenCLDevice.BorrowedCommandQueue(this.commandQueue);
         } else {
            return null;
         }
      }
   ...
   private void returnCommandQueue(long queue) {
      FlowschedAssertions.assertTrue(queue == this.commandQueue);
      synchronized (this.contextMutex) {
      ...
      synchronized (this.contextMutex) {
         if (this.open.compareAndSet(true, false)) {
            ...
            try {
               ...
               LOGGER.error("Failed to release OpenCL command queue for {}", this.deviceDescription, var6);
            }

            try {
               int[] refcnt = new int[1];
               CLUtil.checkCLError(CL12.clGetContextInfo(this.context, 4224, refcnt, null));
               if (refcnt[0] > 1) {
                  LOGGER.warn("OpenCL context for {} is still referenced by {} objects", this.deviceDescription, refcnt[0] - 1);
               }
            } catch (Throwable var5) {
               LOGGER.error("Failed to query refcnt for {}", this.deviceDescription, var5);
            }

            try {
               CLUtil.checkCLError(CL12.clReleaseContext(this.context));
            } catch (Throwable var4) {
               LOGGER.error("Failed to release OpenCL context for {}", this.deviceDescription, var4);
            }
   ...
   public class BorrowedCommandQueue implements Closeable {
      ...
      private BorrowedCommandQueue(long commandQueue) {
         Objects.requireNonNull(OpenCLDevice.this);
      ...
      public void close() {
         if (this.open.compareAndSet(true, false)) {
            ...
            OpenCLDevice.LOGGER.warn("Attempted to close OpenCL command queue twice for {}", OpenCLDevice.this.deviceDescription);
         }
      }
   }
}
